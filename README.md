...
Stack: Python (scraping + clustering) → Node.js/Express (API) → React + Vite (frontend)

What it does
Pulls recent articles from BBC, NPR, and The Guardian RSS feeds
Groups related articles into "clusters" using keyword overlap — no ML involved
...
This project has three parts that need to be set up in order: the scraper, the backend, and the frontend.
...
It's safe to re-run — duplicate articles are skipped automatically.
...
The backend reads scraper/news.db in read-only mode and serves the API at:
...
GET /timeline	All clusters, shaped for the timeline chart
GET /clusters/:id	Full article list for one cluster
POST /ingest/trigger	Re-runs the scraper
GET /ingest/status/:jobId	Polls the status of a triggered scrape

The ingestion endpoints are implemented and tested locally. They are not currently wired to a frontend refresh button (see Design decisions below).
...
text
scraper/   Python: fetches RSS feeds, normalizes and stores articles in SQLite,
           then clusters related stories
              ↓
backend/   Node/Express: read-only API over the SQLite database,
           plus an endpoint to trigger a fresh scrape
              ↓
frontend/  React + Vite: fetches API data and renders the timeline,
           cluster details, and source filtering

Data flows one direction — the frontend never accesses the database directly.

Design decisions & trade-offs

The project was intentionally kept simple and explainable rather than adding complexity that wasn't necessary for the core functionality.
